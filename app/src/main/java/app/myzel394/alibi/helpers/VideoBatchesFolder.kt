...
class VideoBatchesFolder(
		...
		return when (type) {
			...
			BatchType.CUSTOM -> {
				val file = customFolder!!.findFile(fileName)
					?: customFolder.createFile("video/$extension", fileName)!!
				file.uri.toString()
			}
	...
	override fun cleanup() {
		runCatching { customParcelFileDescriptor?.close() }
	}
	...
	fun asMediaGetScopedStorageContentValues(name: String) = ContentValues().apply {
		...
		put(MediaStore.Video.Media.RELATIVE_PATH, SCOPED_STORAGE_RELATIVE_PATH)
